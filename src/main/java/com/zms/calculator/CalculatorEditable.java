...
class CalculatorEditable extends SpannableStringBuilder {
    ...
    replace(int start, int end, CharSequence tb, int tbstart, int tbend) {
        if (isInsideReplace) {
            ...
            ///M: when exceed max length then vibrate @{
            if (origLength + deltaLength > MAX_INPUT_LENGTH) {
                int remainLength = MAX_INPUT_LENGTH - origLength;
                Calculator.log("The Edit view has already contains " + origLength + " chars, "
                        + "and has accepted only " + remainLength + " chars, " + "while the input length is actually "
                        + deltaLength);
                Calculator.vibrate();
